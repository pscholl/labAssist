...
public class AttachmentUploader
{
    ...
    throws UploadException, InterruptedException
  {
    ...
    if (paramArrayOfByte != null)
    {
      ...
      localAttachmentInsertRequest.setMimeType(paramString1);
      localAttachmentInsertRequest.setContent(paramArrayOfByte);
      localAttachmentInsertRequest.setName(paramString2);
      localAttachmentInsertRequest.setCreationTime(paramLong);
      localAttachmentInsertRequest.setSource(paramString3);
      if (!Assert.isTest())
        localAttachmentInsertRequest.setDeviceId(new SettingsSecure(this.context.getContentResolver()).getString("android_id"));
    ...
    label211: for (int j = 4; ; j = localProtoResponse.getErrorCode())
    {
      throw new UploadException(j, paramString1, "File " + paramString2 + " failed to upload.");
      i = 0;
      break;
    }
    label221: SyncNano.AttachmentInsertResponse localAttachmentInsertResponse = (SyncNano.AttachmentInsertResponse)localProtoResponse.getResponseProto();
    if (localAttachmentInsertResponse.getResponseCode() == 0)
    {
      String str = localAttachmentInsertResponse.getAttachmentId();
      FormattingLogger localFormattingLogger2 = logger;
      Object[] arrayOfObject2 = new Object[2];
      arrayOfObject2[0] = localAttachmentInsertResponse.getAttachmentId();
      arrayOfObject2[1] = str;
      localFormattingLogger2.w("Successfully inserted attachment to server: %s, %s", arrayOfObject2);
      if (str != null)
        return str;
    }
    throw new UploadException(4, paramString1, "File " + paramString2 + " failed to upload with server response code " + localAttachmentInsertResponse.getResponseCode());
  }
