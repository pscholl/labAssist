...
public class OverscrollView extends ScrollView
{
  ...
  public OverscrollView(Context paramContext, AttributeSet paramAttributeSet)
  {
    ...
    this.itemSnapAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener()
    {
      ...
      {
        OverscrollView.this.setItem(((Float)paramAnonymousValueAnimator.getAnimatedValue()).floatValue());
      }
    });
  }
  ...
  public void scrollToItem(float paramFloat)
  {
    ...
    float f1;
    for (float f2 = Math.min(Math.max(paramFloat, -0.49F), 0.49F + (-1 + getNumberOfItems())); ; f2 = Math.min(Math.max(paramFloat, 0.0F), Math.max(getNumberOfItems() - f1, 0.0F)))
    {
      setItem(f2);
      return;
 ...
 * Qualified Name:     com.google.glass.voice.menu.OverscrollView
