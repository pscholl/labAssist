...
public class HtmlRenderer
{
  ...
  private final SparseArray<Canvas> pageCanvases = new SparseArray();
  private OnPageCountChangeListener pageChangeListener;
  private final SparseArray<OnRenderListener> pageListeners = new SparseArray();
  ...
  private void render(final TimelineNano.TimelineItem paramTimelineItem, final int paramInt, boolean paramBoolean1, final boolean paramBoolean2, OnPageCountChangeListener paramOnPageCountChangeListener)
  {
    ...
    this.itemModifiedTime = paramTimelineItem.getModifiedTime();
    this.webView.setWebViewClient(new ResourceLoadingWebViewClient(this.context, paramTimelineItem, this.bitmapFactory, MapHelper.getInstance(this.context), MosaicHelper.getInstance(), new Thumbnailer(), paramBoolean1, this.resourceLoadTasks, new ResourceLoadingWebViewClient.ResourceLoadedCallback()
    {
    ...
    if (Log.isLoggable(TAG, 2))
      this.webView.setWebChromeClient(new WebChromeClient()
      {
        public void onConsoleMessage(String paramAnonymousString1, int paramAnonymousInt, String paramAnonymousString2)
        {
          ...
          localFormattingLogger.v("%s %s", arrayOfObject);
        }
      });
    ...
    if ((TimelineHelper.hasAttachmentOfTypes(paramTimelineItem, new String[] { "application/glass+html" })) && (paramTimelineItem.getSourceType() == 1))
  ...
  private void renderToImage(WebView paramWebView, int paramInt)
  {
    ...
    int i;
    int j;
    label122: UserEventHelper localUserEventHelper;
    ...
    if ((!this.onlyLoadFirstPage) && (this.hasCoverPage))
    {
      ...
      if ((Labs.isEnabled(Labs.Feature.WK_CH)) && (paramInt == 0))
      {
    ...
    label298: label304: for (boolean bool = true; ; bool = false)
    {
  ...
  {
    String str = paramTimelineItem.getId() + ':' + paramTimelineItem.getModifiedTime();
    ...
    localStringBuilder1.append("<script>");
    localStringBuilder1.append("AutoSizer.init();");
    localStringBuilder1.append("UrlUniquifier.init('" + str + "');");
    localStringBuilder1.append("</script>");
    ...
    localStringBuilder2.append(paramTimelineItem.getHtml());
