...
public class ResourceLoadingWebViewClient extends WebViewClient
{
  ...
  public ResourceLoadingWebViewClient(Context paramContext, TimelineNano.TimelineItem paramTimelineItem, CachedBitmapFactory paramCachedBitmapFactory, MapHelper paramMapHelper, MosaicHelper paramMosaicHelper, Thumbnailer paramThumbnailer, boolean paramBoolean, List<AsyncTask<Void, Void, Void>> paramList, ResourceLoadedCallback paramResourceLoadedCallback)
  {
    ...
    this.thumbnailer = paramThumbnailer;
  ...
  private TimelineNano.Attachment findAttachmentForAttachmentUri(Uri paramUri)
  {
    ...
    {
      logger.v("Ignoring non-uniquified URI: " + paramUri, new Object[0]);
      return null;
    }
    ...
    if (TextUtils.isEmpty(str))
    {
      ...
      arrayOfObject3[1] = this.item.getId();
    ...
    try
    {
      ...
      if ((i < 0) || ((this.item.attachment != null) && (i >= this.item.attachment.length)))
      {
        ...
        arrayOfObject2[1] = this.item.getId();
    ...
    catch (NumberFormatException localNumberFormatException)
    {
      ...
      arrayOfObject1[1] = this.item.getId();
  ...
  private TimelineNano.Attachment findAttachmentForCidUri(Uri paramUri)
  {
    ...
    if (TextUtils.isEmpty(str))
    {
      ...
      arrayOfObject[1] = this.item.getId();
  ...
  private String loadAttachment(final TimelineNano.Attachment paramAttachment, List<Runnable> paramList)
  {
    ...
    switch (6.$SwitchMap$com$google$glass$html$Thumbnailer$ThumbnailerResult[this.thumbnailer.createThumbnail(paramAttachment, str, true).ordinal()])
    {
    default:
  ...
  private byte[] loadMosaic(final Uri paramUri, List<Runnable> paramList)
  {
    byte[] arrayOfByte;
    if (TextUtils.isEmpty(paramUri.getFragment()))
    {
    ...
    final ImageLoader localImageLoader;
    do
    {
      return arrayOfByte;
      localImageLoader = this.mosaicHelper.getImageLoader(this.item, paramUri);
      if (localImageLoader == null)
        return null;
      arrayOfByte = this.mosaicHelper.getCachedMosaicImage(localImageLoader);
    }
    ...
    paramList.add(new Runnable()
    {
      public void run()
      {
        ListenableFuture localListenableFuture = ResourceLoadingWebViewClient.this.mosaicHelper.renderMosaicToJpeg(paramUri, localImageLoader);
        try
        {
          localListenableFuture.get();
          return;
        ...
        catch (ExecutionException localExecutionException)
        {
          ...
          Object[] arrayOfObject = new Object[1];
          arrayOfObject[0] = paramUri;
          localFormattingLogger.e(localExecutionException, "Mosaic rendering error, uri: %s", arrayOfObject);
        }
      }
    });
    return null;
  ...
  public WebResourceResponse shouldInterceptRequest(WebView paramWebView, String paramString)
  {
    ...
    while (true)
    {
      ...
      try
      {
        ...
        label265: if (this.loadUncachedResources)
        {
          ...
          while (true)
            if (localIterator.hasNext())
            {
              ...
              continue;
              label324: if ("attachment".equals(str3))
              {
              ...
              if (!bool2)
                break;
              if (MapHelper.isGlassMapUri(localUri))
              {
              ...
              boolean bool3 = MosaicHelper.isGlassMosaicUri(localUri);
              arrayOfByte = null;
              str1 = null;
              str2 = null;
              if (!bool3)
                break;
              arrayOfByte = loadMosaic(localUri, localArrayList);
              str1 = "image/jpeg";
              str2 = null;
              break;
              localObject = null;
