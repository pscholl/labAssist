...
public abstract class ImageProxyDownloadTask extends DeferredContentLoader.LoadingTask<String>
{
  ...
  private String downloadImage(String paramString)
  {
    ...
    if (this.width != -1)
      localRequestedDimensions.setWidthPixels(this.width);
    if (this.height != -1)
      localRequestedDimensions.setHeightPixels(this.height);
    ImageDownloadNano.ImageDownloadRequest localImageDownloadRequest = new ImageDownloadNano.ImageDownloadRequest();
    localImageDownloadRequest.setRequestedDimensions(localRequestedDimensions);
    localImageDownloadRequest.setUrl(this.imageUrl);
    localImageDownloadRequest.setCropType(this.cropType);
    ...
    if (localImageDownloadResponse.getStatus() != 0)
    {
      FormattingLogger localFormattingLogger4 = logger;
      Object[] arrayOfObject5 = new Object[2];
      arrayOfObject5[0] = Integer.valueOf(localImageDownloadResponse.getStatus());
      arrayOfObject5[1] = this.imageUrl;
      localFormattingLogger4.w("Server status: %s for: %s", arrayOfObject5);
      return null;
    }
    if (ArrayUtils.isEmpty(localImageDownloadResponse.getImage()))
    {
    ...
    arrayOfObject1[0] = "b";
    arrayOfObject1[1] = Integer.toString(localImageDownloadResponse.getImage().length);
    ...
    this.cachedFilesManager.save(CACHE_TYPE, paramString, FileSaver.newSaver(localImageDownloadResponse.getImage()));
    return str2;
  }
