...
public class AttachmentHelper
{
  ...
  public static String getAttachmentSource(Context paramContext, TimelineNano.TimelineItem paramTimelineItem, TimelineNano.Attachment paramAttachment)
  {
    ...
    return paramTimelineItem.getSource();
  }
  ...
  {
    if (paramAttachment.getId().startsWith("ps:"));
    while (((paramAttachment.getClientCachePath() != null) && (paramAttachment.getClientCachePath().startsWith(GlassApplication.DCIM_DIRECTORY))) || (attachmentCreatedOnDevice(paramAttachment.getSource())))
      return true;
    return false;
  }
  ...
  public static List<TimelineNano.Attachment> mergeAttachments(List<TimelineNano.Attachment> paramList1, List<TimelineNano.Attachment> paramList2)
  {
    ...
    while ((i < paramList1.size()) && (i < paramList2.size()))
    {
      ...
      {
        if (((TimelineNano.Attachment)localObject).hasId());
        while (true)
        {
  ...
  {
    return (paramAttachment.getClientCachePath() != null) && (paramAttachment.getClientCachePath().startsWith(GlassApplication.DCIM_DIRECTORY)) && (paramTimelineItem.getSource() != null) && (paramTimelineItem.getSource().startsWith("device:"));
  }
  ...
  public boolean cloneAttachments(TimelineNano.TimelineItem paramTimelineItem)
  {
    ...
    if (l3 < l2)
    {
      ...
      if (!localCachedFilesManager.trimBySize(l5))
      {
        ...
        arrayOfObject7[1] = paramTimelineItem.getId();
    ...
    arrayOfObject1[2] = paramTimelineItem.getId();
    ...
    if (m < n)
    {
      ...
      if (isPhotosServiceAttachment(localAttachment1))
      {
        ...
        arrayOfObject5[0] = localAttachment1.getId();
      ...
      while (true)
      {
        ...
        if (localFile1 == null)
          break label529;
        String str = paramTimelineItem.getId() + "-" + m;
        if (!localCachedFilesManager.save(CachedFilesManager.Type.SHARE, str, FileSaver.newSaver(localFile1)))
          break label466;
        localAttachment1.setClientCachePath(localCachedFilesManager.getPath(CachedFilesManager.Type.SHARE, str));
        localAttachment1.clearId();
      }
      label466: FormattingLogger localFormattingLogger4 = logger;
      Object[] arrayOfObject4 = new Object[2];
      arrayOfObject4[0] = localAttachment1.getId();
      arrayOfObject4[1] = paramTimelineItem.getId();
      ...
      while (true)
      {
        m = i1;
        break;
        label529: FormattingLogger localFormattingLogger3 = logger;
        Object[] arrayOfObject3 = new Object[2];
        arrayOfObject3[0] = localAttachment1.getId();
        arrayOfObject3[1] = paramTimelineItem.getId();
  ...
  {
    Assert.assertNotUiThread();
    if ((!TextUtils.isEmpty(paramAttachment.getClientCachePath())) && (new File(paramAttachment.getClientCachePath()).exists()));
    ...
    for (String str = paramAttachment.getClientCachePath(); ; str = localCachedFilesManager.getPath(paramType, paramAttachment.getId()))
    {
      ...
      while (bool2);
      byte[] arrayOfByte2 = getAttachmentFromServer(paramAttachment.getId());
    ...
    arrayOfObject[0] = paramAttachment.getId();
    ...
    throws InterruptedException
  {
    ...
    localAttachmentGetRequest.setAttachmentId(paramString);
    ...
    if (localWindowManager != null)
    {
      ...
      localScreenDimensions.setWidthPixels(localDisplay.getWidth());
      localScreenDimensions.setHeightPixels(localDisplay.getHeight());
      localAttachmentGetRequest.setScreenDimensions(localScreenDimensions);
    }
    ...
    while (localAttachmentGetResponse.getResponseCode() != 0);
    byte[] arrayOfByte = localAttachmentGetResponse.getContent();
    FormattingLogger localFormattingLogger = logger;
