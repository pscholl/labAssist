...
class SpeakableText
{
  ...
  private String getSpeakableTextWithContext(Context paramContext, TimelineNano.TimelineItem paramTimelineItem)
  {
    ...
    String str2;
    if (paramTimelineItem.hasSpeakableType())
      if (!TextUtils.isEmpty(str1))
        str2 = formatExtraContentDescription(paramContext, paramTimelineItem.getSpeakableType(), str1);
    do
    {
      return str2;
      return paramTimelineItem.getSpeakableType();
      if (!TextUtils.isEmpty(str1))
        return formatExtraContentDescription(paramContext, getGlasswareName(paramContext, paramTimelineItem), str1);
      boolean bool1 = paramTimelineItem.hasHtml();
      str2 = null;
      if (bool1)
      {
        String str3 = Html.fromHtml(stripTags(paramTimelineItem.getHtml(), Arrays.asList(new String[] { "style" }))).toString();
  ...
  String getText(TimelineNano.TimelineItem paramTimelineItem)
  {
    if (paramTimelineItem.hasSpeakableText())
      return paramTimelineItem.getSpeakableText();
    if (paramTimelineItem.hasText())
      return paramTimelineItem.getText();
    return "";
  }
}
