  ...
  implements SensorEventListener, AbsolutePitchHelper.AbsolutePitchListener
{
  ...
  private DoubleBlinkBroadcastReceiver doubleBlinkReceiver = new DoubleBlinkBroadcastReceiver(null);
  ...
  public HeadScroller(Context paramContext, ItemScroller.Scrollable paramScrollable)
  {
    ...
    setupLookUpDownAngles(paramContext);
  ...
  {
    if (EyeGestureUtils.stopEyeGestureDetector(this.context, EyeGesture.WINK))
    {
  ...
  {
    if (EyeGestureUtils.startEyeGestureDetector(this.context, EyeGesture.WINK, false, -1))
    {
  ...
  private void setupLookUpDownAngles(final Context paramContext)
  {
    this.userGluAngle = 30.0F;
    this.lookDownAngle = (this.userGluAngle - 10.0F);
    new AsyncTask()
    {
      Stopwatch stopwatch = new Stopwatch();

      protected Float doInBackground(Void[] paramAnonymousArrayOfVoid)
      {
        this.stopwatch.start();
        return Float.valueOf(HeadScroller.getHeadWakeAngleDegs(paramContext));
      }

      protected void onPostExecute(Float paramAnonymousFloat)
      {
        HeadScroller.access$502(HeadScroller.this, paramAnonymousFloat.floatValue());
        HeadScroller.access$602(HeadScroller.this, HeadScroller.this.userGluAngle - 10.0F);
        FormattingLogger localFormattingLogger = HeadScroller.logger;
        Object[] arrayOfObject = new Object[2];
        arrayOfObject[0] = Float.valueOf(HeadScroller.this.userGluAngle);
        arrayOfObject[1] = Long.valueOf(this.stopwatch.getTotalElapsedMilliseconds());
        localFormattingLogger.v("User's GLU Angle: %f, time to query: %d ms", arrayOfObject);
      }
    }
  ...
  private void setupScreenOffReceiver()
  {
    ...
    {
      this.screenOffReceiver = new ScreenOffBroadcastReceiver(null);
      this.screenOffReceiver.registerAsync(AsyncThreadExecutorManager.getSerialExecutor(), this.context);
    }
  }
  ...
  private void startYawCalculations()
  {
    this.sensorExecutor.execute(new Runnable()
    {
      public void run()
      {
        HeadScroller.access$902(HeadScroller.this, 0L);
  ...
  public void activate()
  {
    if (!this.isActivated)
    {
      ...
      this.doubleBlinkReceiver.registerAsync(AsyncThreadExecutorManager.getSerialExecutor(), this.context);
  ...
  void disableDoubleBlinkDetector()
  {
    ...
    if (EyeGestureUtils.stopEyeGestureDetector(this.context, EyeGesture.DOUBLE_BLINK))
      logger.v("Double blink detector is stopped.", new Object[0]);
    while (true)
    {
      if ((EyeGestureUtils.isEyeGestureDetectorPersistentlyEnabled(this.context, EyeGesture.WINK)) && (!EyeGestureUtils.isEyeGestureDetectorRunning(this.context, EyeGesture.WINK)))
        enableWinkDetector();
      return;
  ...
  void enableDoubleBlinkDetector()
  {
    ...
    if (EyeGestureUtils.startEyeGestureDetector(this.context, EyeGesture.DOUBLE_BLINK, false, -1))
    {
      logger.v("Double blink detector is started.", new Object[0]);
      this.timeToDblBlinkStopWatch.reset();
      if (EyeGestureUtils.isEyeGestureDetectorRunning(this.context, EyeGesture.WINK))
        disableWinkDetector();
      return;
    }
  ...
  private class DoubleBlinkBroadcastReceiver extends SafeBroadcastReceiver
  {
    private DoubleBlinkBroadcastReceiver()
    {
      super();
    }
    ...
    {
      if (EyeGesture.valueOf(paramIntent.getStringExtra("gesture")) == EyeGesture.DOUBLE_BLINK)
  ...
  private class ScreenOffBroadcastReceiver extends SafeBroadcastReceiver
  {
    ...
    public void onReceiveInternal(Context paramContext, Intent paramIntent)
    {
      ...
      {
        HeadScroller.access$1302(HeadScroller.this, false);
        HeadScroller.this.screenOffReceiver.unregisterAsync(AsyncThreadExecutorManager.getSerialExecutor(), paramContext);
        HeadScroller.access$1402(HeadScroller.this, null);
      }
    }
  }
}
