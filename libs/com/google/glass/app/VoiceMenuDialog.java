...
public class VoiceMenuDialog extends Dialog
{
  ...
  private final ConnectivityBroadcastReceiver connectivityReceiver = new ConnectivityBroadcastReceiver(null);
    ...
    return new VoiceInputHelper(paramGlassVoiceActivity, new VoiceListener.SimpleVoiceListener()
    {
      ...
      public VoiceConfig onVoiceCommand(final VoiceCommand paramAnonymousVoiceCommand)
      {
        ...
        VoiceMenuDialog.this.voiceMenuEnvironment.postToUiThread(new Runnable()
        {
          public void run()
          {
            VoiceMenuDialog.2.this.val$activity.onVoiceMenuCommand(paramAnonymousVoiceCommand);
          }
        });
        return localVoiceConfig;
      }
  ...
  {
    super.onStart();
    this.connectivityReceiver.register(this.activity);
    ...
    implements VoiceMenu.VoiceMenuListener
  {
    ...
    public void showPeopleList(VoiceMenuItem paramVoiceMenuItem, EntitySet paramEntitySet, VoiceMenuEnvironment.EntityCommand paramEntityCommand)
    {
      ArrayList localArrayList = Lists.newArrayList();
      VoiceMenuDialog.this.activity.onPrepareVoiceSubmenu(paramVoiceMenuItem, paramEntitySet, paramEntityCommand, localArrayList);
      if (localArrayList.isEmpty())
        return;
      VoiceMenuDialog.this.voiceMenu.selectMenuItem(paramVoiceMenuItem, localArrayList);
    }
