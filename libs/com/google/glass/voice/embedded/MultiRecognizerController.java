...
public class MultiRecognizerController
{
  ...
  private static final String TAG = MultiRecognizerController.class.getSimpleName();
  private static MultiRecognizerController instance = null;
  ...
  private MultiRecognizerController(Context paramContext)
  {
  ...
  private RecognizerCallback createRecognizerCallback()
  {
    return new RecognizerCallback()
    {
      ...
      {
        MultiRecognizerController.logger.i("Endpointer Event: " + paramAnonymousEndpointerEvent, new Object[0]);
      }
      ...
      public void handleRecognitionEvent(RecognizerProtos.RecognitionEvent paramAnonymousRecognitionEvent)
      {
        if ((paramAnonymousRecognitionEvent.hasPartialResult()) && (paramAnonymousRecognitionEvent.getPartialResult().getHotwordFired()) && (paramAnonymousRecognitionEvent.getPartialResult().getPartCount() > 0) && (paramAnonymousRecognitionEvent.getPartialResult().getPart(0).hasText()))
        {
          ...
          MultiRecognizerController.access$202(MultiRecognizerController.this, paramAnonymousRecognitionEvent.getPartialResult());
          MultiRecognizerController.this.setDecoder(MultiRecognizerController.this.currentDecoder);
        }
      }
    };
  }
  ...
  public static MultiRecognizerController getEmbeddedRecognizerController(Context paramContext)
  {
    try
    {
      boolean bool = GRECO_DIR.exists();
      if (!bool);
      for (MultiRecognizerController localMultiRecognizerController = null; ; localMultiRecognizerController = instance)
      {
        return localMultiRecognizerController;
        if (instance == null)
          instance = new MultiRecognizerController(paramContext);
      }
    }
    finally
    {
  ...
  private boolean startRecognition(Decoder paramDecoder)
  {
    ...
    this.recognizerThread = new Thread("RecognizerThread")
    {
      public void run()
      {
        NativeRecognizer.NativeRecognitionResult localNativeRecognitionResult = MultiRecognizerController.this.recognizer.run(localRecognizerSessionParams);
        if (localNativeRecognitionResult.getStatus() != 0)
          MultiRecognizerController.logger.d("GlassMultiRecognizer exited with status: " + localNativeRecognitionResult.getStatus(), new Object[0]);
        MultiRecognizerController.logger.d("run() finished", new Object[0]);
      }
    };
 ...
 * Qualified Name:     com.google.glass.voice.embedded.MultiRecognizerController
