  ...
  implements InputListener, ValueSlider.Listener
{
  ...
  private HeadsetPlugBroadcastReceiver headsetPlugReceiver = new HeadsetPlugBroadcastReceiver(null);
  ...
  private final SafeBroadcastReceiver screenOffReceiver = new SafeBroadcastReceiver(new String[] { "android.intent.action.SCREEN_OFF" })
  {
    public FormattingLogger getLogger()
    {
      return VolumeControlDialog.screenOffReceiverLogger;
    }

    public void onReceiveInternal(Context paramAnonymousContext, Intent paramAnonymousIntent)
    {
      if (("android.intent.action.SCREEN_OFF".equals(paramAnonymousIntent.getAction())) && (VolumeControlDialog.this.isShowing()))
      {
        VolumeControlDialog.logger.d("Cancelling for screen off event.", new Object[0]);
        VolumeControlDialog.this.cancel();
      }
    }
  };
  ...
  {
    super.onStart();
    this.headsetPlugReceiver.registerAsync(AsyncThreadExecutorManager.getSerialExecutor(), getContext());
  ...
  private class HeadsetPlugBroadcastReceiver extends SafeBroadcastReceiver
  {
    ...
    {
      getLogger().d("Received intent: %s", new Object[] { paramIntent });
      if ("android.intent.action.HEADSET_PLUG".equals(paramIntent.getAction()))
        VolumeControlDialog.this.initSlider(VolumeHelper.getHeadsetState(paramIntent));
    }
  }
}
